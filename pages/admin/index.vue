  ...
  <div class="admin-dashboard space-y-8">
    ...
    <div class="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      ...
      <div class="relative w-full sm:w-64 shrink-0">
        <UiIcon icon="heroicons:magnifying-glass" class="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
        <input v-model="searchQuery" type="text" placeholder="Search users by name..." class="w-full pl-9 pr-4 py-2 text-sm border border-slate-200 rounded-lg focus:ring-1 focus:ring-slate-950 outline-none transition-all" />
      </div>
    </div>
    <!-- Main Content -->
      ...
      <tr v-for="user in filteredUsers" :key="user.id" class="hover:bg-slate-50 transition-colors group">
        ...
        <td class="whitespace-nowrap px-3 py-4">
          <div class="flex flex-col gap-1.5">
            ...
            <p v-if="user.subscriptions && user.subscriptions.length && user.subscriptions[0].status === 'ACTIVE' && user.subscriptions[0].subscriptionEnds" class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1">
              Renews: {{ new Date(user.subscriptions[0].subscriptionEnds).toLocaleDateString("en-GB", { day: 'numeric', month: 'short', year: 'numeric' }) }}
            </p>
            <span v-if="user.role === 'ADMIN'" class="text-[9px] font-bold text-emerald-600/80 border border-emerald-100 bg-emerald-50 px-1.5 py-0.5 rounded-md w-fit uppercase tracking-widest mt-1">Admin</span>
          </div>
        </td>
        ...
        <td class="relative whitespace-nowrap py-4 pl-3 pr-6 text-right">
          <UiPopover placement="bottom-end">
            ...
            <template #default="{ close }">
              ...
              <div class="p-1">
                <button @click="close(); openBillingEditor(user)" class="w-full flex items-center gap-2 px-3 py-2 text-xs font-semibold text-slate-600 hover:text-blue-600 hover:bg-blue-50 rounded-md transition-all text-left">
                  <UiIcon icon="heroicons:calendar-days" custom-class="w-4 h-4" />
                  Edit Billing Date
                </button>
                <button @click="close(); toggleUserStatus(user)" class="w-full flex items-center gap-2 px-3 py-2 text-xs font-semibold text-slate-600 hover:text-emerald-600 hover:bg-emerald-50 rounded-md transition-all text-left mt-0.5">
    ...
    <UiModal v-model="editBillingModal.isOpen" title="Edit Billing Date" max-width="max-w-md">
      <div class="p-6">
        <div class="space-y-4">
          <div>
            <label class="block text-[10px] font-semibold text-slate-400 uppercase tracking-widest mb-1.5">User Profile</label>
            <p class="text-sm font-semibold text-slate-900">{{ editBillingModal.user?.name || 'Unnamed' }} ({{ editBillingModal.user?.email }})</p>
          </div>
          <div>
            <label class="block text-[10px] font-semibold text-slate-400 uppercase tracking-widest mb-1.5">New Expiration Date</label>
            <input type="date" v-model="editBillingModal.newDate" class="block w-full rounded-md border border-slate-200 px-3 py-2 text-sm focus:ring-1 focus:ring-slate-950 outline-none transition-all" />
            <p class="text-[11px] text-slate-500 mt-2 leading-relaxed">Setting a future date instantly upgrades and grants the user uninterrupted access to their plan until exactly that day.</p>
          </div>
        </div>
        <div class="mt-8 flex justify-end gap-3">
          <button @click="editBillingModal.isOpen = false" class="px-4 py-2 text-sm font-semibold text-slate-600 hover:text-slate-900 hover:bg-slate-100 rounded-lg transition-colors">Cancel</button>
          <button @click="saveBillingDate" :disabled="savingBilling" class="px-4 py-2 text-sm font-semibold text-white bg-slate-900 hover:bg-slate-800 rounded-lg shadow-sm transition-all flex items-center gap-2 disabled:opacity-50">
            <UiIcon v-if="savingBilling" icon="line-md:loading-twotone-loop" class="w-4 h-4 animate-spin" />
            Save & Extend
          </button>
        </div>
      </div>
...
const openBillingEditor = (user) => {
  ...
  editBillingModal.value.newDate = currentEnd 
    ? new Date(new Date(currentEnd).getTime() + Math.abs(new Date(currentEnd).getTimezoneOffset() * 60000)).toISOString().split('T')[0] 
    : "";
...
const saveBillingDate = async () => {
  ...
  if (success) {
    toast.value = { message: "Billing date successfully extended!", type: "success" };
