<template>
  <div class="dashboard-page w-full w-full mx-auto font-sans pb-8">
    <div class="flex flex-col gap-8">
      <div
        ...
        <div>
          ...
          <p class="text-xs font-medium text-slate-500 mt-1">
            What you are owed, and what is chasing itself.
          </p>
        </div>
      ...
      <!-- ── Money summary ────────────────────────────────────────────────
           Reworked. The four tiles used to be four different pastels — lavender
           for revenue, yellow for outstanding, mint for clients, pink for
           overdue — which is decorative, not semantic: nothing about revenue is
           "indigo". Colour now appears only where it carries meaning, and the
           one number this audience opens the page for (what they are owed) gets
           the hierarchy instead of an equal quarter of a rainbow. -->
      <section class="grid grid-cols-1 lg:grid-cols-3 gap-6" aria-label="Money summary">
        <!-- Lead figure -->
        <div
          class="lg:col-span-2 bg-white border border-slate-200 rounded-2xl p-6 shadow-sm flex flex-col">
          <dt class="text-[11px] font-bold uppercase tracking-wider text-slate-500">
            Outstanding
          </dt>
          <dd
            class="mt-2 text-4xl font-semibold text-slate-900 tracking-tight tabular-nums">
          ...
          <!-- The action, not just the number -->
          <NuxtLink
            v-if="dashboardStore.stats.overdueCount > 0"
            to="/invoices?status=Overdue"
            class="mt-5 flex items-center gap-3 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 transition-colors hover:border-rose-300">
            <UiIcon
              icon="heroicons:exclamation-triangle-solid"
              custom-class="w-5 h-5 text-rose-600 shrink-0" />
            <span class="text-sm font-semibold text-rose-800">
              {{ dashboardStore.stats.overdueCount }}
              {{ dashboardStore.stats.overdueCount === 1 ? "invoice is" : "invoices are" }}
              overdue
            </span>
            <span class="ml-auto text-xs font-semibold text-rose-700">Review &rarr;</span>
          </NuxtLink>
          <div
            v-else
            class="mt-5 flex items-center gap-3 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3">
            <UiIcon
              icon="heroicons:check-circle-solid"
              custom-class="w-5 h-5 text-emerald-600 shrink-0" />
        ...
        <!-- Supporting figures: neutral, because neither is an alarm -->
        <div class="grid grid-cols-2 lg:grid-cols-1 gap-6">
          <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
            <dt class="text-[11px] font-bold uppercase tracking-wider text-slate-500">
              Collected
            </dt>
            <dd class="mt-1.5 text-xl font-semibold text-slate-900 tracking-tight tabular-nums">
              {{ dashboardStore.stats.currency || "MYR" }}
          ...
          <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
            ...
            <dd class="mt-1.5 text-xl font-semibold text-slate-900 tracking-tight tabular-nums">
              {{ dashboardStore.stats.activeClients || 0 }}
            </dd>
          </div>
        </div>
      </section>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div class="lg:col-span-3 space-y-8">
          ...
          <div
            ...
            <!-- Height collapses when there is nothing to plot: a 350px empty
                 box was the largest thing on the page for a new account. -->
            <div
              class="p-6"
              :class="chartData?.datasets?.[0]?.data?.length ? 'h-[350px]' : 'h-[160px]'">
              <ClientOnly>
                <UiChart
                ...
                <div
                  ...
                  <p>No invoices in this range yet — pick a wider range, or send your first invoice</p>
                </div>
              </ClientOnly>
            </div>
          </div>
            ...
            <div
              ...
              <ul role="list" class="space-y-5">
                ...
                <li
                  ...
                  class="py-8 px-4 text-center bg-slate-50 rounded-xl border border-dashed border-slate-200">
        ...
        <div class="lg:col-span-1 space-y-8">
          ...
          <div
            ...
            <p class="text-xs text-slate-600 mb-4 leading-relaxed">
              Share your code and a reward is credited when they subscribe.
            </p>
            ...
            <div class="space-y-4">
              ...
              <div class="flex items-center justify-between px-1">
                <span
                  class="text-xs font-semibold text-slate-600 capitalize tracking-wider"
                  >My Credits</span
                >
      ...
      <!-- ── Plan usage ───────────────────────────────────────────────────
           Moved down. This is account administration — how much of your monthly
           allowance is left — and it was sitting directly under the money
           summary, above the cashflow you actually came to look at. It belongs
      ...
      <div class="bg-white border border-slate-200 shadow-sm rounded-xl p-6">
        <div class="flex items-center justify-between mb-6">
          ...
          <span
            class="text-[10px] font-semibold px-2 py-1 rounded bg-slate-100 text-slate-700 uppercase tracking-widest"
